...
def setup_and_launch_ptf_runner(
    device,
    pipeline_name,
    protobuf_dir,
    ptf_dir,
    ...
    EXTERNAL_PATH_GOOGLE_PROTOBUF = (
        EXEC_ROOT + "/external/" + protobuf_dir + "/google/protobuf"
    )
    ...
    EXTERNAL_PATH_PTF = (
        ...
        + ptf_dir
        + "/"
        + ptf_dir[6:].replace("_", "-", 1).replace("_", ".")
        + ".data/scripts"
    )
    EXTERNAL_PATH_P4_FABRIC_TEST = (
        EXEC_ROOT + "/external/com_github_opennetworkinglab_fabric_p4test/tests/ptf"
    )
    ...
    ptf_setup_cmd = (
        ...
        + EXEC_PATH_PTF
        + " && "
        + "sudo mount -t tmpfs ptf_fs -o size=10M "
        + EXEC_PATH_PTF
        + " && "
        + "sed '1 s/python/\/usr\/bin\/env python3/' "
        + EXTERNAL_PATH_PTF
        + "/ptf > "
        + EXEC_PATH_PTF
        + "/ptf && "
    ...
    ptf_umount_cmd = "sudo umount ptf_fs"
